...
def compute_data_metrics(batch: DataProto, trajectory_turns, use_critic: bool = True, reward_info=None) -> dict[str, Any]:
    """
    ...
    if 'step_reward' in reward_info:
        ...
        # print('reward_info 1',len(reward_info["prm@1"]))
        # print('reward_info 2',len(reward_info["prm@2"]))
        # print('reward_info 3',len(reward_info["prm@3"]))
        # print('reward_info 4',len(reward_info["prm@4"]))
        # exit(0)

        # max_turn = len(reward_info['step_reward'][0])
        max_turn = max(len(x) for x in reward_info['step_reward'])
        orm_score = reward_info["orm"]
        metrics[f"critic/orm/mean"] = float(sum(orm_score)/len(orm_score))
        # print('max_turnmax_turn',max_turn)
        for t in range(0, max_turn):
            succ = reward_info[f"success@{t+1}"]
            metrics[f"critic/success@{t+1}/mean"] = float(sum(succ)/len(succ))
            
            acc = reward_info[f"acc@{t+1}"]
            metrics[f"critic/acc@{t+1}/mean"] = float(sum(acc)/len(acc))
    
            prm = reward_info[f"prm@{t+1}"]
            metrics[f"critic/prm@{t+1}/mean"] = float(sum(prm)/len(prm))

            prm_wo_gamma = reward_info[f"prm_wo_gamma@{t+1}"]
            metrics[f"critic/prm_wo_gamma@{t+1}/mean"] = float(sum(prm_wo_gamma)/len(prm_wo_gamma))

        # print('metrics[f"critic/prm_wo_gamma@{t+1}/mean"]',metrics[f"critic/prm@1/mean"])
        # print('metrics[f"critic/prm_wo_gamma@{t+1}/mean"]',metrics[f"critic/prm@2/mean"])
        # print('metrics[f"critic/prm_wo_gamma@{t+1}/mean"]',metrics[f"critic/prm@3/mean"])
        # print('metrics[f"critic/prm_wo_gamma@{t+1}/mean"]',metrics[f"critic/prm@4/mean"])
        # exit(0)
